...
mp_yearly <- as.data.frame(t(do.call(rbind,MultiYear_EmRems_Values$year[as.character(crediting_period)])))
if ((length(FRLMonitoringPeriodProjectionYears) - length(crediting_period)) > 0) {
  mp_yearly <- 
    cbind(
      mp_yearly,
  ...
  names(mp_yearly) <- as.character(FRLMonitoringPeriodProjectionYears)
}
